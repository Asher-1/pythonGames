...
def isWindow(widget_name):
    """Return true if the widget name is a window type"""
    if widget_name == "GtkWindow":
        return True
    elif widget_name.startswith("Gtk") and widget_name.endswith("Dialog"):
        return True
    else:
        return False  
...
class GladeGen:
    ...
    def __init__(self, module, glade_file, class_name, base_glade_file):
        ...
        self.base_glade_file = base_glade_file
    ...
    def get_xml(self):
        ...
        for node in doc.getElementsByTagName('widget'):
            ...
            if self.top_window is None and isWindow(widget):
    ...
    def get_init_method(self, indent):
        ...
        lines.append("%sfilename = '%s'" % (indent2, self.base_glade_file))
        lines.append('')
...
def main(argv):
    ...
    if len(argv) != 5:
        print 'usage: GladeGen.py glade_file module_name class_name base_glade_file'
        return
    
    glade_file = argv[1]
    module = argv[2]
    class_name = argv[3]
    base_glade_file = argv[4]

    pg = GladeGen(module, glade_file, class_name, base_glade_file)
    pg.get_members()
    pg.get_xml()
