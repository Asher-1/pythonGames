...
class ProbabilityChooser(object):
    ...
    def __init__(self, options, interpolate=False):
        ...
        if isinstance(options, dict):
            ...
            self.total_probability = sum(item[0] for item in options)
            self.items = options
        self.values = []
        #
    ...
    def choose(self):
        ...
        if not self.interpolate or idx == len(self.values) - 1:
            return self.items[idx][1]
        else:
            x1, x2 = self.values[idx:idx + 2]
            y1, y2 = self.items[idx][1], self.items[idx + 1][1]
            #
            if x1 == x2:
                # Can occur if we are right at the end of an interval
                return y1
            else:
                return y1 + (value - x1) / (x2 - x1) * (y2 - y1)

    def chooseMultiple(self, number):
        """Return a number of items chosen at random"""
        return [self.choose() for _ in range(number)]


class ProbabilityFunctionChooser(ProbabilityChooser):
    """Returns values chosen from a range with a probability function"""

    def __init__(self, fn, low=0.0, high=1.0, number=10):
        """Initialise the chooser"""
        #
        # Create the function
        options = []
        for idx in range(number):
            x = low + float(idx) / (number - 1) * (high - low)
            y = fn(x)
            options.append((y, x))
        #
        super(ProbabilityFunctionChooser, self).__init__(options, interpolate=True)

    @staticmethod
    def getTriangle(low, mid, high, multiplier=1.0):
        """Return a triangle function"""
        def fn(x):
            if low <= x <= high:
                if x <= mid:
                    return multiplier * (x - low) / (mid - low)
                else:
                    return multiplier * (high - x) / (high - mid)
            else:
                return 0
        #
        return fn
