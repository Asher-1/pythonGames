...
class Actor(pygame.Rect):
    ...
    def image(self, image):
        ...
        p = self.pos
        self._surf = image if isinstance(image, pygame.Surface) else loaders.images.load(image)
    ...
    def rotate(self, angle):
        self._transform_surface(pygame.transform.rotate, loaders.images.load(self.image), angle)
        if hasattr(self, '_fix_pos'):
            self.pos = self._fix_pos
    ...
    def _transform_surface(self, func, *args):
         p = self.pos
         self._surf = func(*args)
         self.width, self.height = self._surf.get_size()
         self._calc_anchor()
         self.pos = p
    ...
    def draw(self, surface):
        if self.visible:
            surface.blit(self._surf, self.topleft)

    def update(self, dt):
        """Pass through for updates"""
