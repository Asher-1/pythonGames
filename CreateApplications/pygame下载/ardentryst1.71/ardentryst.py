...
shops = {
    "Elchim's Stall": {
        ...
        "Items": ["Potion", "Potion2", "Mana potion", "Mana potion2", "Silver ring", "Luck charm", "Steel gladius",
                  "Steel cap", "Bronze battle axe2", "Steel halberd2", "Wingboots"],
        "ExchangeRate": 1.1
    },
    "Elchim's Shop": {
        "Shopfront": "elchim.png",
        ...
        "Items": ["Potion", "Potion2", "Mana potion", "Mana potion2", "Silver ring", "Luck charm", "Oak staff",
                  "Blue hood", "Magic crossbow2", "Enchanted sickle2", "Wingboots"],
        "ExchangeRate": 1.1
    },
    "Ferna's Stall": {
        ...
        "Items": ["Potion3", "Potion4", "Mana potion3", "Mana potion4", "Antidote", "Magic crossbow2",
                  "Magic crossbow3", "Bronze battle axe3", "Ancient pendant", "Wingboots", "Gripboots", "Rage pendant",
                  "Nature pendant", "Confusion pendant", "Enchanted sickle2", "Enchanted sickle3"],
        "ExchangeRate": 1.1
    },
    ...
    "Tyra's Shop": {
        ...
        "Items": ["Crescent staff", "Broadsword", "Green hood", "Wingboots", "Gripboots", "Sorcerers hat",
                  "Steel round helm", "Anneludine platebody", "Anneludine shell platelegs", "Nepthene platebody",
                  "Frost robe", "Frost skirt", "Steel halberd3", "Sorcerers robe", "Sorcerers skirt"],
        "ExchangeRate": 1.1
    }
}
...
def useshop(shop, game, player):
    ...
    while Inshop:
        ...
        if Buy or Sell:
            ...
            if Howmany <= -1:
                pc_s = Fonts[16].render(
                    "Press " + namekey("B-1") + " to " + ["buy", "sell"][Sell] + " the selected item", 1,
                    (255, 255, 255))
                pc_r = pc_s.get_rect()
                pc_r.center = (477, 310)
                screen.blit(pc_s, pc_r)

                pb_s = Fonts[16].render("Press " + namekey("B-2") + " to get out of " + ["Buy", "Sell"][Sell] + " mode",
                                        1, (255, 255, 255))
                pb_r = pb_s.get_rect()
                pb_r.center = (477, 325)
        ...
        if Buy:
            ...
            if Howmany > -1:
                ...
                hms = Fonts[16].render(
                    "Costs " + str(Howmany * int(Inventory[selection].value * shop["ExchangeRate"])) + "s", 1,
                    (255, 255, 255))
                hmr = hms.get_rect()
                hmr.center = (495, 415)
                screen.blit(hms, hmr)
            ...
            if Howmany > -1:
                screen.fill((0, 0, 0), Rect(420, 395, 150, 40))
                hms = Fonts[16].render(
                    "Worth " + str(Howmany * int(pinvd[pinvdl[selection]][1].value / shop["ExchangeRate"])) + "s", 1,
                    (255, 255, 255))
                hmr = hms.get_rect()
                hmr.center = (495, 415)
                screen.blit(hms, hmr)
            ...
            if ev.type == KEYDOWN:
                ...
                if k in p1c["Right"]:
                    ...
                    for key in ["strength", "endurance", "magic", "luck"]:
                        if selobj.usage_bonus[key]: information.append(
                            key.capitalize() + " " + strbon(selobj.usage_bonus[key]))
...
class ac:
    # Controls AC
    def __init__(self):
        self.cf = [os.path.join("esaB"[::-1], x) for x in os.listdir("esaB"[::-1])] + [os.path.join("sleveL"[::-1], x)
                                                                                       for x in
                                                                                       os.listdir("sleveL"[::-1])]
    ...
    def cds(self):
        ...
        for f in self.cf:
            try:
                sh = getattr(mys.new(getattr(mym.new(open(f, "r").read()), "tsegidxeh"[::-1])()[::-1]),
                             "tsegidxeh"[::-1])()
                bh += sh
                if self.dgests[f] != sh:
                    logfile(chr(42) + " " + f + ".erocs ruoy daolpu ot elba eb ton lliw uoY .elif lanigiro eht ton si "[
                                                ::-1])
                    rv = len(f) ^ len(f)
...
def Game_SlotMenu(gameobj=None):
    ...
    gamelist.sort(
        lambda x, y: cPickle.load(open(os.path.join(SAVEDIRECTORY, "Saves", x), "r")).startdate < cPickle.load(
            open(os.path.join(SAVEDIRECTORY, "Saves", y), "r")).startdate)
    ...
    while inslotmenu:
        ...
        for x in range(len(gamelist)):
            ...
            gsw = Fonts[16].render(
                torender + ["", "_"][newgame and (tick / 20) % 2 and x == cursor and not passwordstage], 1,
                (255, 255, 255))
            gsb = Fonts[16].render(
                torender + ["", "_"][newgame and (tick / 20) % 2 and x == cursor and not passwordstage], 1, (0, 0, 0))
            gsr = gsw.get_rect()
        ...
        if not newgame:
            ...
            charname = Fonts[16].render("(" + [gameobj.character, "..."][gameobj.character == "Nobody"] + ")", 1,
                                        (255, 255, 255))
            ...
            if gameobj.location[1]:
                place = "in " + mapdata[0][gameobj.location[0]]["name"] + ": " + \
                        mapdata[gameobj.location[0]][gameobj.location[1]]["name"]
            else:
...
def choose_character(screen):
    ...
    while True:
        ...
        for stat in stats:
            ...
            screen.fill((255 - (255.0 / maxstat) * stat * 0.75, (255.0 / maxstat) * stat * 0.75, 0),
                        statrect.inflate(6, 6))
...
def design_character(Game, player):
    ...
    while ALLOTPHASE:
        ...
        if P_REM == 12:
            status = "Right/left keys adds/subtracts points and up/down changes the selection."
        elif P_REM > 0:
            status = "Completely distribute all the points."
        elif P_REM == 0:
            status = "Press Return or " + namekey("B-1") + " to finalise the distribution!"
...
def handle_game(Game, loaded=False):
    ...
    while True:
        ...
        if Game.world_tut <= 1:
            if Game.location == [1, 0]:
                worldbox_msg = "This is the map of Ardentryst. This overview of the region shows you all the great locations in the continent. For now, we start in Sempridge. Press " + namekey(
                    "B-1") + " to enter Sempridge."
                worldbox_pos = 330
            elif Game.location == [2, 0]:
                worldbox_msg = "This is the map of Ardentryst. This overview of the region shows you all the great locations in the continent. For now, we start in Entarya. Press " + namekey(
                    "B-1") + " to enter Entarya."
                worldbox_pos = 330
        ...
        if Game.location == [1, 1] and Game.world_tut <= 1:
            worldbox_msg = "This is the forest town of Sempridge. We must first start here in this location. Press " + namekey(
                "B-1") + " to enter this area...."
            worldbox_pos = 150
            if Game.world_tut == 0:
                worldbox_surfs = []
            Game.world_tut = 1
        ...
        if Game.location == [2, 1] and Game.world_tut <= 1:
            worldbox_msg = "This is Queen Amee's castle! We must first start here in this location. Press " + namekey(
                "B-1") + " to enter...."
            worldbox_pos = 150
            if Game.world_tut == 0:
                worldbox_surfs = []
        ...
        if Game.world_tut == 2:
            if Game.location == [1, 1]:
                worldbox_msg = "Now the level name at the bottom shows us our level completion percentage. Let's move on. Visit the next place by moving to it, press " + namekey(
                    "Up") + " to get there!"
                worldbox_pos = 150
            elif Game.location == [1, 2]:
                worldbox_msg = "Enter this area if you wish to save your game. Press " + namekey(
                    "B-1") + " to go here. Move on to the next area by proceeding east by pressing " + namekey(
                    "Right") + "!"
                worldbox_pos = 150
        ...
        if Game.location[1] == 0:
            ...
            for location in mapdata[Game.location[0]][1:]:
                if markdata.has_key(location["type"]):
                    if mapdata[Game.location[0]].index(location) in Game.Accessible[Game.location[0]]:
                        ...
                        if Game.scores[Game.location[0] - 1][
                            mapdata[Game.location[0]].index(location) - 1] >= 100: mtype = "Mark4.png"
                        if Game.scores[Game.location[0] - 1][
                            mapdata[Game.location[0]].index(location) - 1] >= 200: mtype = "Mark6.png"
            ...
            if Ondata["type"] == "SAVE":
                ...
                if Game.location[0] >= 3:
                    ...
                    placename += " ... " + \
                                 {0: "No Gem (Slow)", 1: "Yellow (Fast)", 2: "Red (Faster)", 3: "Blue (Fastest)"}[tg]
            if Ondata["map"]:
        ...
        if oldpos != playerpos:
            ...
            if cloudlayer:
                if Game.ENDSCENE:
                    ...
                    screen.blit(Data.images["kiripan.png"][0],
                                (290, 141 - Game.KIRI_HEIGHT * 60 + math.sin(tick / 30.0) * 6))
                else:
            ...
            for x in range([20, 80][60 > Game.ENDSCENE > 0]):
                ...
                if flipme:
                    screen.blit(pygame.transform.flip(Data.images[mylocmark][0], True, False),
                                (int(playerpos[0]) - 16, int(playerpos[1]) - 44))
                else:
            ...
            if Ondata["up"]:
                for entry in [mapdata[Game.location[0]][1:], mapdata[0][1:]][Game.location[1] == 0]:
                    if entry["name"] == Ondata["up"]:
                        ...
                        if [mapdata[Game.location[0]], mapdata[0]][Game.location[1] == 0].index(entry) in \
                                Game.Accessible[lookin] or entry.has_key("type") and entry["type"] == "EXIT_WORLD":
                ...
                if Game.location[1] == 0:
                    ...
                    if event.key in p1c["Up"]:
                        if Ondata["up"]:
                            for entry in mapdata[Game.location[0]][1:]:
                                if entry["name"] == Ondata["up"]:
                                    oldpos = playerpos[:]
                                    if mapdata[Game.location[0]].index(entry) in Game.Accessible[Game.location[0]] or \
                                            entry["type"] == "EXIT_WORLD":
                    ...
                    if event.key in p1c["Left"]:
                        ...
                        if leveltype == "SAVE":
                            ...
                            fade_to_black(screen)
                            Result = playlevel(player, level, [levelscript, npcscript], screen, Data, Fonts, soundbox,
                                               Game,
                                               [g_options,
                                                a_options,
                                                p_options,
                            ...
                            if Result[0] == "Quit":
                                ...
                                if mapdata[Game.location[0]][Game.location[1]]["nextloc"] and Result[1] != -1:
                                    for x in range(1, len(mapdata[Game.location[0]])):
                                        if mapdata[Game.location[0]][x]["name"] == \
                                                mapdata[Game.location[0]][Game.location[1]]["nextloc"]:
                                            Game.location[1] = x
                                            Game.Accessible[Game.location[0]].append(x)
                                            break
...
def OptionsScreen(o_g_options, o_a_options, o_p_options, p1c):
    ...
    while OptionsScreenActive:
        ...
        if tab == "Graphics":
            ...
            if h_key:
                keysurf = Fonts[13].render(h_key.binding.replace("B-", "Action button ") + ": " + h_key.label, 1,
                                           (255, 255, 255))
...
def main():
    ...
    try:
        ...
        raise Exception(
            "** Please remove your options.dat file (in your ~/.ardentryst/ directory) and run Ardentryst again. **")
    except IOError:
    ...
    while GAMELOOP:
        ...
        LIGHTNINGCUES = [[879, 160], [1059, 200], [1488, 150], [2007, 170], [13890, 240], [14700, 130], [15350, 120],
                         [16000, 100], [40592, 255], [41700, 100], [42486, 70], [43524, 70], [54394, 120], [61790, 50],
                         [81900, 250], [82000, 180], [82100, 160], [83200, 100], [83400, 100], [83600, 90], [83800, 80],
                         [85605, 80], [95798, 255], [96800, 200], [97920, 120], [98820, 100], [116996, 100]]
        DONELIGHTNINGCUES = []
        if SOUND:
        ...
        while ON_MENU:
            ...
            if SOUND:
                ...
                if len(DONELIGHTNINGCUES) < len(LIGHTNINGCUES):
                    ...
                    if music_ms > LIGHTNINGCUES[len(DONELIGHTNINGCUES)][0]:
                        ...
                        LIGHTNING.append([random.randint(0, 1), random.randint(0, 640), random.randint(0, 30),
                                          LIGHTNINGCUES[len(DONELIGHTNINGCUES) - 1][1]])
            ...
            # Flying birds
            screen.blit(Data.images["mbirds" + str(abs(ticker / 10 % 6 - 3) + 1) + ".png"][0],
                        ((ticker / 1.8) % 2000 - 180, 125 + math.sin(ticker / 50.0) * 15))
            screen.blit(Data.images["mbirds" + str(abs((ticker - 18) / 10 % 6 - 3) + 1) + ".png"][0],
                        ((ticker / 1.9) % 2000 - 130, 120 + math.sin((ticker - 15) / 59.0) * 12))
            screen.blit(Data.images["mbirds" + str(abs((ticker - 39) / 10 % 6 - 3) + 1) + ".png"][0],
                        ((ticker / 2) % 2000 - 60, 110 + math.sin((ticker - 8) / 75.0) * 19))
            screen.blit(Data.images["mbirds" + str(abs(ticker / 10 % 6 - 3) + 1) + ".png"][0],
                        ((ticker / 1.8) % 2000 - 680, 125 + math.sin(ticker / 50.0) * 15))
            screen.blit(Data.images["mbirds" + str(abs((ticker - 18) / 10 % 6 - 3) + 1) + ".png"][0],
                        ((ticker / 1.9) % 2000 - 730, 120 + math.sin((ticker - 15) / 59.0) * 12))
            screen.blit(Data.images["mbirds" + str(abs((ticker - 39) / 10 % 6 - 3) + 1) + ".png"][0],
                        ((ticker / 2) % 2000 - 860, 110 + math.sin((ticker - 8) / 75.0) * 19))
            ...
            if selected == "Quit":
                ...
                if rv:
                    ...
                    if oops[0]["Fullscreen"] + g_options["Fullscreen"] == 1 or \
                            oops[0]["Widescreen"] + g_options[
                        "Widescreen"] == 1:  # I.e. "Fullscreen" or "Widescreen" has changed
                ...
                while listing:
                    ...
                    lc_s = Fonts[17].render(str(max(0, int(scroll))) + "/" + str(len(source)) + " lines of code", 1,
                                            (255, 255, 255))
                    lc_r = lc_s.get_rect()
...
class Spark:
    ...
    def __init__(self, pos, tick):
        tick += random.randint(0,
                               40)  # This makes some out of time, as in some are red before they're supposed to be etc
        self.x, self.y = pos
