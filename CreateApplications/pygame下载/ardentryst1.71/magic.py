...
class Fire_1(Spell):
    ...
    def s_blit(self, surf, ALT_X, ALT_Y):
        ...
        for flame in self.flame_entities:
            ...
            rect.midbottom = (self.x + flame * 8, self.y - math.sin(flame / 2.0) * 5.0 + c * self.rise + (
                        (c + 0.001) ** (abs(self.rise) + 1)) * [1, -1][self.rise < 0])
            rect.move_ip(ALT_X, ALT_Y)
            grect.midbottom = (self.x + flame * 8, self.y - math.sin(flame / 2.0) * 5.0 + c * self.rise + (
                        (c + 0.001) ** (abs(self.rise) + 1)) * [1, -1][self.rise < 0])
    ...
    def stick(self):
        ...
        self.flame_entities = [[x + self.mydir, -40][[x < -35 or x > 0, x > 35 or x < 0][self.unchdir]] for x in
                               self.flame_entities]
...
class Burst(Spell):
    ...
    def s_blit(self, surf, ALT_X, ALT_Y):
        ...
        msurf = pygame.transform.scale(pic, (
        max(5, min(185, self.mticker ** 3)) + 15, max(5, min(185, self.mticker ** 3)) + 15))
...
class Ice_1(Spell):
    ...
    def s_blit(self, surf, ALT_X, ALT_Y):
        ...
        if not self.affected:
            ...
            return
        self.mticker = -abs(self.ticker - 15) + 15
        msurf = pygame.transform.scale(DATA.mag_images["ice.png"][0], (
        max(5, min(125, self.mticker ** 2)) + 15, max(5, min(125, self.mticker ** 2)) + 15))
...
class Ice_2(Ice_1):
    ...
    def s_blit(self, surf, ALT_X, ALT_Y):
        ...
        if not self.affected:
            ...
            return
        self.mticker = -abs(self.ticker - 15) + 15
        msurf = pygame.transform.scale(DATA.mag_images["ice2.png"][0], (
        max(5, min(125, self.mticker ** 2)) + 15, max(5, min(125, self.mticker ** 2)) + 15))
        msurf = pygame.transform.rotate(msurf, self.ticker ** 1.7)
        rect = msurf.get_rect()
        for mon in self.affected:
...
class Fire_2(Spell):
    ...
    def s_blit(self, surf, ALT_X, ALT_Y):
        ...
        for flame in self.flame_entities:
            ...
            gimg, grect = DATA.mag_images["m_fire1_glow.png"]
            img, rect = DATA.mag_images[self.frames[flame % 5]]
            rect.midbottom = (self.x + flame * 4, self.y - math.sin(flame / 2.0) * 5.0 + c * self.rise + (
                        (c + 0.001) ** (abs(self.rise) + 1)) * [1, -1][self.rise < 0])
            rect.move_ip(ALT_X, ALT_Y)
            grect.midbottom = (self.x + flame * 4, self.y - math.sin(flame / 2.0) * 5.0 + c * self.rise + (
                        (c + 0.001) ** (abs(self.rise) + 1)) * [1, -1][self.rise < 0])
            grect.move_ip(ALT_X, ALT_Y + 3)
            for y in range(-1, 2):
    ...
    def stick(self):
        ...
        self.flame_entities = [[x + self.mydir, -666][[x < -65 or x > 0, x > 65 or x < 0][self.unchdir]] for x in
                               self.flame_entities]
