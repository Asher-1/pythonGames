...
class pod(Monster):
    ...
    def collision(self, entity):
        if entity.x > self.x:
            entity.inertia[0] = random.randint(3, 6)
        else:
            entity.inertia[0] = random.randint(-6, -3)
...
class giantjelly(Monster):
    ...
    def collision(self, entity):
        if entity.x > self.x:
            entity.inertia[0] = random.randint(4, 8); self.inert -= .5
        else:
            entity.inertia[0] = random.randint(-8, -4); self.inert += .5
        if not self.hittime:
...
class worm(Monster):
    ...
    def collision(self, entity):
        if entity.x > self.x:
            entity.inertia[0] = random.randint(2, 4); self.inert -= 2
        else:
            entity.inertia[0] = random.randint(-4, -2); self.inert += 2
        if not self.hittime:
            entity.raw_hit(self.pain * (random.randint(60, 140) / 100.0))
            self.hittime = 20
            self.SOUND = "collide"
...
class wasp(Monster):
    ...
    def collision(self, entity):
        if entity.x > self.x:
            entity.inertia[0] = random.randint(1, 2)
        else:
            entity.inertia[0] = random.randint(-2, -1)
        if not self.hittime:
            entity.raw_hit(self.pain * (random.randint(60, 140) / 100.0))
            self.hittime = 12
            self.SOUND = "hurt"
...
class giantworm(Monster):
    ...
    def collision(self, entity):
        if entity.x > self.x:
            entity.inertia[0] = random.randint(10, 16); self.inert = 0
        else:
            entity.inertia[0] = random.randint(-16, -10); self.inert = 0
        if not self.hittime:
            entity.raw_hit(self.pain * (random.randint(60, 140) / 100.0))
            self.hittime = 30
            self.SOUND = "collide"
...
class snowogre(Monster):
    ...
    def collision(self, entity):
        if self.state == 0: return
        if self.state == 1 or self.state == 2: entity.inertia[0] = (((int(
            entity.x > self.x)) * 2) - 1) * 3; self.inert = 0; return

        # is lunging
        if entity.x > self.x:
            entity.inertia[0] = random.randint(2, 4)
        else:
            entity.inertia[0] = random.randint(-4, -2)
        if not self.hittime:
            entity.raw_hit(self.pain * (random.randint(60, 140) / 100.0))
            self.hittime = 20
        #            self.SOUND = "collide"
