...
# windowSurface = pygame.display.set_mode((w, h), FULLSCREEN, 32)
...
for file_name in os.listdir('images'):
    ...
    for file_name in os.listdir('images/' + str(file_name)):
        ...
        image = pygame.image.load(
            'images/' + sub1 + '/' + str(i) + '.' + fileN[len(fileN) - 3] + fileN[len(fileN) - 2] + fileN[
                len(fileN) - 1]).convert()
        images[int(sub1)].append(image)
        i += 1
        currentUpdate()
        progress(current, fileNumber_total, image,
...
for file_name in os.listdir('planets'):
    ...
    for file_name in os.listdir('planets/' + str(file_name)):
        fileN = str(file_name)
        image = pygame.image.load(
            'planets/' + sub1 + '/' + str(i) + '.' + fileN[len(fileN) - 3] + fileN[len(fileN) - 2] + fileN[
                len(fileN) - 1]).convert()
    ...
    def returnDir(self):
        ...
        if (self.dirX <= self.maxSpeed / 2 and self.dirX >= -self.maxSpeed / 2 and self.dirY > 0):
            ...
            self.dir = "down"
                ...
                self.dirX <= self.maxSpeed + 1 and self.dirX >= -self.maxSpeed / 2 and self.dirY <= self.maxSpeed / 2 and self.dirY >= -self.maxSpeed / 2):
            self.dir = "left"
        elif (
                self.dirX >= -self.maxSpeed - 1 and self.dirX <= -self.maxSpeed / 2 and self.dirY <= self.maxSpeed / 2 and self.dirY >= -self.maxSpeed / 2):
            self.dir = "right"
...
asteroids_name = ["1 Ceres", "4 Vesta", "2 Pallas", "10 Hygiea", "704 Interamnia", "52 Europa", "511 Davida",
                  "65 Cybele", "15 Eunomia", "3 Juno", "31 Euphrosyne", "624 Hektor", "88 Thisbe", "324 Bamberga",
                  "451 Patienta",
                  "532 Herculina", "48 Doris", "375 Ursula", "107 Camilla", "45 Eugenia", "7 Iris", "29 Amphirite",
                  "423 Diotima",
                  "19 Fortuna", "13 Egeria", "24 Themis", "94 Aurora", "702, Alauda", "121 Hermione", "Aletheia",
                  "372 Palma", "128 Nemenios"]
...
class Asteroid:
    ...
    def show(self):
        ...
        if (self.toDisplay):
            ...
            pygame.draw.polygon(windowSurface, self.color,
                                [self.v1, self.v2, self.v3, self.v4, self.v5, self.v6, self.v7, self.v8], 4)
            if (self.size > 100):
    ...
    def health(self):
        if (self.maxlife > 1):
            ...
            pygame.draw.rect(windowSurface, GREEN, (
                self.mostInLeft, self.bestY, health * (self.mostInRight - self.mostInLeft) / 70, self.size / 3))
        ...
        for i in range(len(asteroids)):
            if (asteroids[i].y > -self.zoomH and asteroids[i].y < self.zoomH):
                if (asteroids[i].x < self.zoomW or asteroids[i].x > -self.zoomW):
                    ...
                    pygame.draw.circle(windowSurface, asteroids[i].color,
                                       (int(x * 3.32 / 3.425), int(y * 3.32 / 3.425)), int(size))
        for i in range(len(planets)):
...
while True:
    ...
    if (ship.altitude > 10000 and len(planets) < 80):
        planets.append(
            Planet(random.randint(-w * 50, w * 50), random.randint(-h * 100, -h * 4), random.randint(500, 800)))
    ...
    for i in range(len(asteroids)):
        ...
        if (math.fabs(distFromCenterX) > render_distance[0] * 1.4 and len(asteroids) > 1 or math.fabs(distFromCenterY) >
                render_distance[1] * 1.4 and len(asteroids) > 1):
            index_asteroid_to_remove.append(i)
        if (asteroids[i].maxlife != asteroids[i].life):
            asteroids[i].health()
        if (asteroids[i].x < 0 and asteroids[i].x > -w * 2):
            pygame.draw.circle(windowSurface, asteroids[i].color, (0, int(asteroids[i].y)),
                               int(math.fabs(distFromCenterX) / 100), 1)
        elif (asteroids[i].x > w and asteroids[i].x < w * 2):
            pygame.draw.circle(windowSurface, asteroids[i].color, (w - 10, int(asteroids[i].y)),
                               int(math.fabs(distFromCenterX) / 100), 1)
        elif (asteroids[i].y < 0 and asteroids[i].y > -h * 2):
            pygame.draw.circle(windowSurface, asteroids[i].color, (int(asteroids[i].x), 10),
                               int(math.fabs(distFromCenterY) / 100), 1)
        elif (asteroids[i].y > h and asteroids[i].y < h * 2):
            pygame.draw.circle(windowSurface, asteroids[i].color, (int(asteroids[i].x), h - 10),
                               int(math.fabs(distFromCenterY) / 100), 1)
        ...
        if (ship.x + 31 < asteroids[i].v4[0] and ship.x + 31 > asteroids[i].v1[0] and ship.y + 31 > asteroids[i].v2[
            1] and ship.y + 31 < asteroids[i].v7[1]):
    ...
    for i in range(len(bullets)):
        ...
        for j in range(len(asteroids)):
            if (bullets[i].x >= asteroids[j].v1[0] and bullets[i].x <= asteroids[j].v4[0] and bullets[i].y >=
                    asteroids[j].v2[1] and bullets[i].y <= asteroids[j].v7[1]):
                # random.choice(expl_sounds).play()
