...
# 0~3: player 0~3
mjloc = [(300, 815-dis), (1100, 120), (250, 90), (50, 120)]
huloc = [(575, 630), (910, 425), (575, 270), (240, 425)]

# only for first_hear
hear_loc = [(1100, 800-dis), (1150, 70), (180, 40), (10, 800-dis)]
p0_is_AI = False
Add_Delay = True
p0_get_loc_org = (880, 815-dis)
p0_get_loc = []
eat_index = []
...
button_loc = [(1000, 800-dis), (1050, 800-dis), (1100, 800-dis), (1000, 850-dis), (1050, 850-dis), (1100, 850-dis)]
...
drop_mj = [[], [], [], []]
lloc = [(410, 710-dis), (1155, 400), (750, 205), (5, 400)]
hostloc = [(360, 710-dis), (1155, 450), (800, 205), (5, 450)]
hmj_loc = [[(460, 700-dis)], [(985, 320)], [(460, 205)], [(165, 320)]]
htext_loc = [(750, 700-dis), (950, 270), (380, 205), (165, 270)]
hmj = [[], [], [], []]
...
bool_last_one = False
add_kong_loc = [[], [], [], []]
dmj_loc = [[(280, 755-dis)], [(1040, 150)], [(280, 150)], [(110, 150)]]
...
def seq3_block(mj, mj_num, block):
    ...
    while i < mj_num - 2:
        if 1 == block[i]:
            ...
            if m1 != -1 and m2 != -1 and mj[i] < 27 and mj[i] // 9 == mj[m1] // 9 == mj[m2] // 9 and mj[i] + 1 == mj[
                m1] and mj[m1] + 1 == mj[m2]:
    ...
    while i < mj_num - 1:
        if mj[i] >= 27:
            ...
            if mj[i] // 9 == mj[j] // 9 == value // 9:
                if mj[i] + 2 == mj[j] + 1 == value or mj[i] + 2 == value + 1 == mj[j] or value + 2 == mj[i] + 1 == mj[
                    j]:
                    eindex.append(i)
                    eindex.append(j)
                    break
            ...
            while i < mj_num:
                ...
                if -1 == n1 and -1 == n2:
                    ...
                    if mj[i] == mj[n1] == mj[n2]:
                        ...
                        m1, m2 = next_two_not_blsame(block, mj_num, i + 1, mj, mj[i])
                        if m1 != -1 and m2 != -1:
                            if mj[i] < 27 and mj[i] // 9 == mj[m1] // 9 == mj[m2] // 9 and mj[i] + 1 == mj[m1] and mj[
                                m1] + 1 == mj[m2]:
                                block[i] = block[m1] = block[m2] = 1
                                i += 1
                                continue
...
def handle_hu(hid, drop_id=-1, get_hu=True, akong=None, hhu=False):
    ...
    if -1 == drop_id:
        result = hu_result.hu_result(player_mj[hid], dmj[hid], host_num, first_turn[hid], hmj[hid], circle,
                                     player_door[hid], bool_last_one, getmj, first_hear[hid], None, hhu, False,
                                     bool_pre_kong)
    else:
        if akong != None:
        ...
        result = hu_result.hu_result(player_mj[hid], dmj[hid], host_num, first_turn[hid], hmj[hid], circle,
                                     player_door[hid], bool_last_one, None, first_hear[hid], dp, hhu, bool_akong, False)
...
def draw_mj_column(mj_pic, xy, mj_num, draw_all=-1):
    ...
    if -1 == draw_all:
        ...
        if True == gethu:
            screen.blit(pid_to_image(draw_all, getmj),
                        (startx, starty + gap + t1.get_width() + (mj_num - 1) * p0_mj_width))
            ...
            if 0 == pid and -1 == win_id and add_kong_mj != None:
                ...
                if x < mouseX < x + 2 * p0_mj_width + mjbk.get_width() and y < mouseY < y + mjbk.get_height():
                    pygame.draw.rect(screen, (0xff, 0, 0),
                                     (x, y, 2 * p0_mj_width + mjbk.get_width(), mjbk.get_height()), 3)
...
def draw_text():
    screen.blit(write(u"%s风%s局" % (wind_index_to_text(circle), wind_index_to_text(player_door[host_id])), (255, 255, 255)), wind_loc)
    screen.blit(write(u"麻将剩余:%d" % (mjb - mjp + 1), (255, 255, 255)), renum_loc)


def write(msg=u"pygame is cool", color=(0, 0, 0), size=36):
...
def draw_ctai(r):
    ...
    for key, value in r.table.items():
        if value != 0:
            s += value
            screen.blit(write(u"%s: %2d台\n" % ((key).encode('utf-8'), value), (255, 255, 255)), (x, y))
    ...
    screen.blit(write(u"總共: %3d台\n" % (s).encode('utf-8'), (255, 255, 255)), (850, y + 40))
        ...
        while (mjb - mjp) >= 16:
            ...
            if True == p0_is_AI:
                ...
                for p in range(4):
                    if player_mj_num[p] != len(player_mj[p]):
                        input("Len error!")
                    if player_mj_num[p] != 16 and player_mj_num[p] != 13 and player_mj_num[p] != 10 and player_mj_num[
                        p] != 7 and player_mj_num[p] != 4 and player_mj_num[p] != 1:
                        input("Num error!")
                # End auto debug
            ...
            if False == p0_is_AI and 0 == turn_id:
                ...
                if True == hear_status[turn_id] and 0 == button_enable[4]:
                    player_mj[turn_id], player_mj_num[turn_id], get_done[turn_id], dk_value = hear_dark_kong(
                        player_mj[turn_id], player_mj_num[turn_id], getmj, htext_loc[turn_id])
                        ...
                        while True:
                            if did == turn_id:
                                ...
                                if check_button < 2:  # check_button == 0 or 1
                                    check_p0_button(player_mj[did], player_mj_num[did], myvalue=None, dj=None,
                                                    value=dmj[turn_id][add_kong_mj][1][0], chk_eat=False,
                                                    chk_huonly=True)
                                    check_button = 2
                        ...
                        if event.type == QUIT:
                            ...
                            if select != None and button_enable[1] != 2:
                                ...
                                if 99 == select:
                                    ...
                                    player_mj[turn_id], player_mj_num[turn_id] = hu_result.insert_mj(getmj,
                                                                                                     player_mj[turn_id])
                                    ebutton = check_p0_button(player_mj[turn_id], player_mj_num[turn_id])
                                    get_done[turn_id] = 2
                                    first_turn[turn_id] += 1
                            ...
                            if True == button_enable_chk():
                                ...
                                if 4 == bselect:  # if hu and 0 == turn_id
                                    ...
                                    if p0_add_kong(dmj[turn_id], player_mj[turn_id], getmj, add_kong_mj) != None:
                                        vi, player_mj[turn_id], player_mj_num[turn_id] = p0_add_kong(dmj[turn_id],
                                                                                                     player_mj[turn_id],
                                                                                                     getmj, add_kong_mj)
            ...
            while 0 == handle_drop_done or 1 == handle_drop_done or 4 == handle_drop_done:
                    ...
                    if did == turn_id:
                        ...
                        while hid != turn_id:
                            if False == p0_is_AI and 0 == hid:
                                if check_button < 2:  # check_button == 0 or 1
                                    check_p0_button(player_mj[hid], player_mj_num[hid], myvalue=None, dj=None,
                                                    value=drop_mj[turn_id][-1])
                                    check_button = 2
                ...
                if True == br:
                    ...
                    if False == p0_is_AI and 0 == did:
                        if check_button < 4:
                            check_p0_button(player_mj[did], player_mj_num[did], myvalue=None, dj=None,
                                            value=drop_mj[turn_id][-1], chk_eat=True)
                            check_button = 4
                                        ...
                                        if event.type == QUIT:
                                            ...
                                            if select != None and 2 == button_enable[3]:  # eat
                                                if None == smj:
                                                    ...
                                                    if player_mj[did][select] < 27 and sort_value[0] // 9 == sort_value[
                                                        1] // 9 == drop_mj[turn_id][-1] // 9 and (
                                                            sort_value[0] + 2 == sort_value[1] + 1 == drop_mj[turn_id][
                                                        -1] or sort_value[0] + 2 == drop_mj[turn_id][-1] + 1 ==
                                                            sort_value[1] or drop_mj[turn_id][-1] + 2 == sort_value[
                                                                0] + 1 == sort_value[1]):
