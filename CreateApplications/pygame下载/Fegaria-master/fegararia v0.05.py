...
def loadLightingImages():
    specialLightingTilesheet = pygame.transform.scale(pygame.image.load("Textures/special lighting tilesheet.png"),
...
def loadItemImages():
    global itemImages
    itemTilesheet = pygame.transform.scale(pygame.image.load("Textures/itemTilesheet.png"),
...
def loadCharacterAnimation():
    ...
    characterimages = pygame.transform.scale(pygame.image.load("Textures/player tilesheet.png"),
...
def loadBackTileImages():
    global backImages
    backTilesheet = pygame.transform.scale(pygame.image.load("Textures/backTilesheet.png"),
                                           (BLOCKSIZE * 16, BLOCKSIZE * 16))
    backImages = []
    for j in range(16):
        for i in range(16):
...
class NPC():
    ...
    def getCollide(self, handle=True):
        ...
        for i in range(3):
            for j in range(3):
                if mapData[int(blockpos[1] + j - 1 - CHUNKSIZE)][int(blockpos[0] + i - 1 - CHUNKSIZE)][0] > 0:
    ...
    def damage(self, val, crit):
        if crit:
            ...
            col = (200, 100, 0)
        damagePopUps.append(
            [val, col, 100, (self.pos[0] + random.randint(-20, 20), self.pos[1] + 30 + random.randint(-20, 20))])
        self.hp -= val
        if self.hp <= 0:
            self.kill()
...
class Bird(NPC):
    ...
    def draw(self):
        screen.blit(birdImages[self.animationFrame + self.direction * 2 + self.col * 4],
                    (self.pos[0] - CAM.pos[0], self.pos[1] - CAM.pos[1]))
        if showHitBoxes:
...
class WorldItem():
    ...
    def update(self):
        ...
        for i in range(3):
            for j in range(3):
                val = mapData[int(blockpos[1] + j - 1 - CHUNKSIZE)][int(blockpos[0] + i - 1 - CHUNKSIZE)][0]
...
class Projectile():
    ...
    def update(self):
        if "arrow" in self.ammoinfo[1]:
            ...
            self.angle += 10
        self.vel = (
            self.vel[0] * self.stats["air resistance"],
            self.vel[1] * self.stats["air resistance"] + self.stats["gravity"])
        self.pos = (self.pos[0] + self.vel[0], self.pos[1] + self.vel[1])
        ...
        try:
            for i in range(3):
                for j in range(3):
                    ...
                    if val not in uncollidableBlocks:
                        blockrect = Rect(BLOCKSIZE * (blockpos[0] + i - 1), BLOCKSIZE * (blockpos[1] + j - 1),
                                         BLOCKSIZE, BLOCKSIZE)
                        if blockrect.colliderect(self.rect):
                            deltaX = self.rect.centerx - blockrect.centerx
                            deltaY = self.rect.centery - blockrect.centery
    ...
    def draw(self):
        ...
        screen.blit(self.surf,
                    (self.pos[0] - self.rect.width / 2 - CAM.pos[0], self.pos[1] - self.rect.height / 2 - CAM.pos[1]))
        if showHitBoxes:
            pygame.draw.rect(screen, (255, 0, 0),
                             Rect(self.rect.left - CAM.pos[0], self.rect.top - CAM.pos[1], self.rect.width,
                                  self.rect.height), 2)
            ...
            for j in range(len(self.blocks[i])):
                if self.blocks[i][j].val in transparentBlocks:
                    if self.blocks[i][j].backval > 0:
                        self.surface.blit(backImages[self.blocks[i][j].backval],
                ...
                if CAM.Map.chunks[chunkPos[1]][chunkPos[0]].blocks[inChunkPos[0]][inChunkPos[1]].backintegrity <= 0:
                    ...
                    for i in range(20):
                        ...
                        if mapData[pos[1]][pos[0]][1] == 20 or mapData[pos[1]][pos[0]][1] == 21:
                            ...
                            WorldItem("wood", random.randint(2, 3), (
                                (pos[0] + CHUNKSIZE) * BLOCKSIZE + BLOCKSIZE,
                                (pos[1] + CHUNKSIZE) * BLOCKSIZE + BLOCKSIZE))
                            if random.randint(0, 7) == 0:
                                WorldItem("acorn", 1, ((pos[0] + CHUNKSIZE) * BLOCKSIZE + BLOCKSIZE,
                                                       (pos[1] + CHUNKSIZE) * BLOCKSIZE + BLOCKSIZE))
                        else:
        ...
        if not tpressed:
            if "backwall" not in tags:
                if not Rect(p.pos[0] - CAM.pos[0] - BLOCKSIZE / 2, p.pos[1] - CAM.pos[1] - BLOCKSIZE, BLOCKSIZE,
                    ...
                    inChunkPos = ((actualPos[0] - chunkPos[0] * CHUNKSIZE * BLOCKSIZE) // BLOCKSIZE,
                                  (actualPos[1] - chunkPos[1] * CHUNKSIZE * BLOCKSIZE) // BLOCKSIZE)
                    if mapData[(actualPos[1] // BLOCKSIZE) - CHUNKSIZE][(actualPos[0] // BLOCKSIZE) - CHUNKSIZE][
                        0] == 0:
                        canPlace = False
                        if mapData[(actualPos[1] // BLOCKSIZE) - CHUNKSIZE][(actualPos[0] // BLOCKSIZE) - CHUNKSIZE][
                            1] != 0:
                            canPlace = True
                                ...
                                mapData[(actualPos[1] // BLOCKSIZE) - CHUNKSIZE - 1][
                            ...
                            canPlace = True
                                ...
                                mapData[(actualPos[1] // BLOCKSIZE) - CHUNKSIZE][
                        ...
                        if canPlace:
                            ...
                            CAM.Map.chunks[chunkPos[1]][chunkPos[0]].updateSurface()
                            mapData[(actualPos[1] // BLOCKSIZE) - CHUNKSIZE][(actualPos[0] // BLOCKSIZE) - CHUNKSIZE][
                                0] = val
                            return True
            else:
                val = getValFromName(name)
                ...
                if mapData[(actualPos[1] // BLOCKSIZE) - CHUNKSIZE][(actualPos[0] // BLOCKSIZE) - CHUNKSIZE][1] == 0:
                    canPlace = False
                    if mapData[(actualPos[1] // BLOCKSIZE) - CHUNKSIZE - 1][(actualPos[0] // BLOCKSIZE) - CHUNKSIZE][
                        1] != 0:
                        canPlace = True
                    elif mapData[(actualPos[1] // BLOCKSIZE) - CHUNKSIZE + 1][(actualPos[0] // BLOCKSIZE) - CHUNKSIZE][
                        1] != 0:
                        canPlace = True
                    elif mapData[(actualPos[1] // BLOCKSIZE) - CHUNKSIZE][(actualPos[0] // BLOCKSIZE) - CHUNKSIZE - 1][
                        1] != 0:
                        canPlace = True
                    elif mapData[(actualPos[1] // BLOCKSIZE) - CHUNKSIZE][(actualPos[0] // BLOCKSIZE) - CHUNKSIZE + 1][
                        1] != 0:
...
class Player():
            ...
            for i in range(len(anvilRecipies)):
                ...
                for k in range(len(anvilRecipies[i][2])):
                    ...
                    if partscomplete >= len(anvilRecipies[i][2]):
                        self.craftableItems.append(
                            [Item(anvilRecipies[i][0], anvilRecipies[i][1]), anvilRecipies[i][2]])
            ...
            for i in range(len(tableRecipies)):
                ...
                for k in range(len(tableRecipies[i][2])):
                    ...
                    if partscomplete >= len(tableRecipies[i][2]):
                        self.craftableItems.append(
                            [Item(tableRecipies[i][0], tableRecipies[i][1]), tableRecipies[i][2]])
            ...
            for i in range(len(furnaceRecipies)):
                ...
                for k in range(len(furnaceRecipies[i][2])):
                    ...
                    if partscomplete >= len(furnaceRecipies[i][2]):
                        self.craftableItems.append(
                            [Item(furnaceRecipies[i][0], furnaceRecipies[i][1]), furnaceRecipies[i][2]])
        found = False
        if findItem != None:
                            ...
                            if self.hotbar[i].name == itemHolding.name:
                                if "tool" not in self.hotbar[i].tags:
                                    ...
                                    if self.hotbar[i].amnt > 999:
                                        self.changeItem(self.hotbar[i].name, self.hotbar[i].tags,
                                    ...
                                    if self.inventory[i][j].name == itemHolding.name:
                                        if "tool" not in self.inventory[i][j].tags:
                                            ...
                                            if self.inventory[i][j].amnt > 999:
                                                self.changeItem(self.inventory[i][j].name, self.inventory[i][j].tags,
                                ...
                                if self.coinSlots[i].name == itemHolding.name:
                                    if "tool" not in self.coinSlots[i].tags:
                                        ...
                                        if self.coinSlots[i].amnt > 999:
                                            self.changeItem(self.coinSlots[i].name, self.coinSlots[i].tags,
                                ...
                                if self.ammoSlots[i].name == itemHolding.name:
                                    if "tool" not in self.ammoSlots[i].tags:
                                        ...
                                        if self.ammoSlots[i].amnt > 999:
                                            self.changeItem(self.ammoSlots[i].name, self.ammoSlots[i].tags,
                                        ...
                                        if self.chestItems[i][j].name == itemHolding.name:
                                            if "tool" not in self.chestItems[i][j].tags:
                                                ...
                                                if self.chestItems[i][j].amnt > 999:
                                                    self.changeItem(self.chestItems[i][j].name,
                                                                    self.chestItems[i][j].tags,
    ...
    def damage(self, val, crit):
        ...
        if self.alive:
            if crit:
                ...
                col = (200, 100, 0)
            damagePopUps.append(
                [val, col, 100, (p.pos[0] + random.randint(-20, 20), p.pos[1] - 80 + random.randint(-20, 20))])
            self.hp -= val
            if self.hp <= 0:
                self.kill()
    ...
    def update(self):
        ...
        for i in range(3):
            for j in range(3):
                val = mapData[int(self.blockpos[1] + j - 1 - CHUNKSIZE)][int(self.blockpos[0] + i - 1 - CHUNKSIZE)][0]
                if val == 84:
    ...
    def draw(self):
        screen.blit(characterFrames[self.animationFrame + self.direction * 4],
...
def pot(pos):
    for i in range(10):
        if mapData[pos[1]][pos[0]][0] == 0 and mapData[pos[1] + 1][pos[0]][0] > 0 and mapData[pos[1] + 1][pos[0]][
            0] not in uncollidableBlocks:
...
while 1:
    ...
    if movingDown:
        if movingDownTimer > 0:
            ...
            if movingDownTimer <= 0:
                movingDown = False
        movingDownTimer
    CAM.pos = (CAM.pos[0] + (p.pos[0] - screenW / 2 - CAM.pos[0]) * 0.05,
               CAM.pos[1] + (p.pos[1] - screenH / 2 - CAM.pos[1]) * 0.05)
    ...
    if pygame.mouse.get_pressed()[0] and p.alive:
        if p.hotbar[p.selectedItem] != None:
            ...
            if not t2pressed:
                t2pressed = True
                stats = {"bounce": False, "life": 300, "damaging": False, "crit": False, "gravity": 0.2, "age": False,
                         "dropammo": False, "air resistance": 0.99}
                if "throwable" in tags:
                    ...
                    if "grenade" in tags:
                        ...
                        stats["bounce"] = True
                        stats["bounceNum"] = 10
                        Projectile(p.pos, vel, [p.hotbar[p.selectedItem].name, p.hotbar[p.selectedItem].tags], stats,
                                   20, p.hotbar[p.selectedItem].imgIndex)
                    else:
                        stats["damaging"] = True
                        stats["dropammo"] = True
                        Projectile(p.pos, vel, [p.hotbar[p.selectedItem].name, p.hotbar[p.selectedItem].tags], stats,
    ...
    fps = clock.get_fps()
    text = font.render(
        str(int(fps)) + "fps  " + str(int(p.pos[0] // BLOCKSIZE)) + "x " + str(int(p.pos[1] // BLOCKSIZE)) + "y", True,
        (255, 255, 255))
